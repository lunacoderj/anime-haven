...
const firebaseErrorMap: Record<string, string> = {
  ...
  "auth/popup-blocked": "Please allow popups for Google login",
...
const AuthPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    catch (err) { const msg = getErrorMessage(err); if (msg) setError(msg); }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try { await signup(email, password, { displayName: username || `${firstName} ${lastName}`.trim() }); nav("/home"); }
    catch (err) { const msg = getErrorMessage(err); if (msg) setError(msg); }
    finally { setIsLoading(false); }
  };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-md rounded-2xl border border-border bg-card p-8">
        ...
        {tab === "login" ? (
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" disabled={isLoading} className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2.5 font-semibold text-primary-foreground hover:bg-primary/80 disabled:opacity-50">
        ...
        <button onClick={handleGoogle} disabled={isLoading} className="flex w-full items-center justify-center gap-2 rounded-lg border border-border bg-muted/50 py-2.5 text-sm font-medium text-foreground hover:bg-muted disabled:opacity-50">
